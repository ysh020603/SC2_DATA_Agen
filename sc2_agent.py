...
def run_agent(
    ...
    if normalized_version in {"v2.2", "v2_2"}:
        from sc2_agents.v2_2.agent import run_agent as run_v2_2_agent

        return run_v2_2_agent(
            user_query,
            provider=provider,
            model=model,
            data_path=data_path,
    ...
    if normalized_version != "v1":
        raise ValueError(f"Unknown agent_version: {agent_version}. Expected 'v1', 'v2', 'v2.1', or 'v2.2'.")
...
def main() -> None:
    ...
    parser.add_argument("--agent-version", choices=["v1", "v2", "v2.1", "v2.2"], default="v2")
